...
function get_monthly_quota($username) {
    try {
        ...
        $sql = "SELECT monthly_exercise_quota AS quota FROM user WHERE username=?";
        $sth = $dbh->prepare($sql);
...
function get_monthly_calories($username, $month) {
    try {
        ...
        // retrieve user_id from database
        $sql = "SELECT id,monthly_exercise_quota AS quota FROM user WHERE username=?";
        $sth = $dbh->prepare($sql);
        $success = $sth->execute(array($username));
        $user_row = $sth->fetch(PDO::FETCH_ASSOC);
        ...
        $sql = "SELECT SUM(calories) AS calories FROM exercise_log WHERE user_id=? AND date_logged BETWEEN ? AND ?";
        $sth = $dbh->prepare($sql);
...
function get_total_calories($username) {
    try {
        ...
        $sql = "SELECT SUM(calories) FROM exercise_log WHERE user_id=?";
        $sth = $dbh->prepare($sql);
...
function add_exercise_entry($username, $password, $calories, $comment) {
    try {
        ...
        $sql = "INSERT INTO exercise_log VALUES(NULL, ?, ?, ?, NULL)";
        $sth = $dbh->prepare($sql);
...
function delete_exercise_entry($username, $password, $id) {
    try {
        ...
        $sql = "DELETE FROM exercise_log WHERE user_id=? AND id=?";
        $sth = $dbh->prepare($sql);
...
if (isset($_GET["method"]) && $_GET["method"] === "add_exercise_entry") {
    if (isset($_POST["username"]) === FALSE) {
        $result = "No username supplied!";
        ...
        $result = add_exercise_entry($username, $password, $calories, $comment);
    }
...
if (isset($_GET["method"]) && $_GET["method"] === "delete_exercise_entry") {
    if (isset($_POST["username"]) === FALSE) {
        $result = "No username supplied!";
    } else if (isset($_POST["password"]) === FALSE) {
        $result = "No password supplied!";
        ...
        $password = $_POST["password"];
        $id = $_POST["id"];
        $result = delete_exercise_entry($username, $password, $id);
    }
